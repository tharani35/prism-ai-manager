...
export function Navbar() {
  ...
  return (
    ...
    >
      ...
      {open && (
        <div className="md:hidden border-t border-white/5 bg-[#020617]/90 backdrop-blur-xl px-6 py-4 flex flex-col gap-4">
          {nav.map((n) => (
            <a key={n.label} href={n.href} className="text-sm text-slate-300" onClick={() => setOpen(false)}>
              {n.label}
            </a>
          ))}
          <a href="#cta" className="rounded-full px-4 py-2 text-sm font-medium text-white text-center"
            style={{ background: "linear-gradient(135deg,#3B82F6,#8B5CF6)" }}>
            Start free trial
          </a>
        </div>
